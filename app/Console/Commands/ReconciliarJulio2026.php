...
class ReconciliarJulio2026 extends Command
{
    protected $signature = 'reconciliar:julio {--apply : Ejecuta los cambios (por defecto es simulación)}';
    protected $description = 'Registra la factura de julio 2026 faltante para los clientes que pagaron julio fuera del sistema';

    private const PERIODO = '2026-07';

    public function handle(FacturaService $facturaService): int
    {
        $apply = (bool) $this->option('apply');
        $numeros = require database_path('data/reconciliacion-julio-2026.php');
        $numeros = array_values(array_unique(array_map('strval', $numeros)));

        $this->info(($apply ? 'APLICANDO' : 'SIMULACIÓN (dry-run)') . ' — reconciliación julio 2026');
        ...
        foreach ($numeros as $num) {
            ...
            if (! $apply) {
                $this->line("  #$num  crearía factura julio = \${$mensualidad}");
                $creadas++;
                continue;
            }
            ...
            try {
                $req = Request::create('/reconciliar', 'POST', [
                    ...
                    'payload'         => [
                        ...
                        'label'            => 'Reconciliación julio 2026',
                        'metodo_pago'      => 'Reconciliación',
                        'metodo'           => 'Reconciliación',
                        'reconciliacion'   => 'julio-2026',
                    ],
                ]);
        ...
        $this->line('Ya tenían julio (saltadas): ' . $yaTenian);
        $this->line('No existen: ' . $noExisten);
        $this->line('Errores: ' . $errores);
        if (! $apply) {
            $this->newLine();
            $this->comment('Simulación. Para aplicar: php artisan reconciliar:julio --apply');
        }
