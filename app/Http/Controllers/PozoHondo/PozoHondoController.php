...
class PozoHondoController extends Controller
{
    ...
    public function exportarCorteExcel(Request $request)
    {
        $user = $request->user();
        $zona = 'pozo_hondo';
        $zonaNombre = 'Pozo Hondo';

        // Buscar corte activo
        $corte = CorteCaja::obtenerActivo($zona, $user->id);

        if (! $corte) {
            return redirect()->route('pozo_hondo.corte')->with('error', 'No hay un corte activo para exportar.');
        }
        ...
        $headers = [
            ...
            'Content-Disposition' => 'attachment; filename="corte-caja-' . $corte->id . '-' . now()->format('Y-m-d') . '.xls"',
