<x-app-layout title="CORTES">
    ...
    <div class="py-6" x-data="cortesManager()">
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                    <div class="flex items-center gap-4">
                        <form action="{{ route('pagos.cortes.index') }}" method="GET" class="flex gap-2">
                    ...
                    <div class="flex gap-2">
                        <a href="{{ route('pagos.cortes.export.pdf') }}?{{ http_build_query(request()->only(['q', 'zona', 'estado'])) }}" class="btn btn-danger flex items-center gap-2" title="Exportar a PDF los usuarios por cortar">
                            📄 PDF
                        </a>
                        <a href="{{ route('pagos.cortes.export.csv') }}?{{ http_build_query(request()->only(['q', 'zona', 'estado'])) }}" class="btn btn-success flex items-center gap-2" title="Exportar a Excel (CSV) los usuarios por cortar">
                            📊 Excel
                        </a>
        ...
        <div x-show="openConfigModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-xs p-4 overflow-hidden">
                ...
                <div class="mb-4">
                    ...
                    <div class="flex gap-2">
                        ...
                        <button @click="addCortador()" 
                            class="btn btn-primary px-4 py-2 rounded-full hover:bg-primary-600 transition-all duration-200">
                            Añadir
                        </button>
                    </div>
                </div>
        ...
        function cortesManager() {
            return {
                ...
                async addCortador() {
                    ...
                    try {
                        const token = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
                        const r = await fetch("{{ route('pagos.cortes.cortadores.store') }}", {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'Accept': 'application/json',
                ...
                async removeCortador(id) {
                    ...
                    try {
                        const token = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
                        // Usamos una URL base de la ruta y concatenamos el ID en JS
                        const url = "{{ route('pagos.cortes.cortadores.destroy', ['id' => 'PLACEHOLDER']) }}".replace('PLACEHOLDER', id);
