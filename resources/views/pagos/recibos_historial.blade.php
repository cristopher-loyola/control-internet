<x-app-layout title="Historial de recibos">
    ...
    <div class="py-6" x-data="{ cancelId:null, motivo:'', metodoId:null, metodoActual:'', nuevoMetodo:'' }">
                                    ...
                                    <tr class="{{ $r->deleted_at ? 'bg-rose-50/80 dark:bg-rose-900/20' : '' }}">
                                        ...
                                        <td class="px-4 py-2 whitespace-nowrap text-sm text-right">
                                            @if(!$r->deleted_at)
                                                ...
                                                <button type="button" class="btn btn-sm ms-1 text-white" style="background:#f97316" x-on:click.prevent="metodoId={{ $r->id }}; metodoActual='{{ addslashes($r->metodo ?? 'Efectivo') }}'; nuevoMetodo='{{ addslashes($r->metodo ?? 'Efectivo') }}'; $dispatch('open-modal','pagos-recibos-metodo')">Método</button>
        ...
        <x-modal name="pagos-recibos-metodo" maxWidth="sm" focusable>
            <form method="POST" :action="metodoId ? '{{ route('pagos.recibos.facturas.update-metodo', ['id'=>'__ID__']) }}'.replace('__ID__', metodoId) : '#'" class="p-6">
                @csrf
                ...
                <div>
                    <x-input-label for="nuevo_metodo_p" value="Nuevo método" />
                    <select id="nuevo_metodo_p" name="metodo" class="form-select mt-1 block w-full" x-model="nuevoMetodo">
