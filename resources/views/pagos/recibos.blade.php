    ...
    <div class="md:col-span-2 bg-gray-50 dark:bg-gray-700 rounded-xl p-5 shadow-inner">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    <button type="button"
                        ...
                        @click="form.metodo = 'Tarjeta de Crédito'">
                    ...
                    <button type="button"
                        ...
                        :class="form.metodo === 'Deposito a cuenta'
                            ...
                            : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-200 border-gray-300 dark:border-gray-600 hover:border-indigo-400'"
                        :disabled="readOnlyMode"
                        @click="form.metodo = 'Deposito a cuenta'">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" aria-hidden="true">
            ...
            <div class="sm:col-span-2">
                ...
                <div class="grid grid-cols-3 sm:grid-cols-5 gap-1.5 max-w-md">
                    <template x-for="(c, idx) in cobradores" :key="c.id">
                        <button type="button"
                            ...
                            :class="form.cobro === c.nombre ? 'bg-indigo-600 text-white border-indigo-600 shadow' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-200 border-gray-300 dark:border-gray-600 hover:border-indigo-400'"
                            :disabled="readOnlyMode" @click="form.cobro = c.nombre; inputChanged(true)">
                            <span class="block text-2xl font-bold leading-none mb-1.5"
                                :class="form.cobro === c.nombre ? 'text-white' : 'text-gray-700 dark:text-gray-100'">
...
<div x-show="pagadoMesActual || alCorriente || (adeudo !== null && adeudo.meses <= 0 && adeudo.pendiente <= 0)" class="mt-4 mb-4 p-4 bg-green-50 border border-green-200 rounded-lg not-print">
            ...
            <div class="px-5 pb-4 flex justify-end gap-2">
                ...
                <button class="btn btn-primary" @click="applyDiscount()">Aplicar</button>
                    ...
                    <div class="mt-6 print-sheet" x-ref="sheet" x-show="layoutReady" x-cloak>
                        ...
                        <div x-show="printFailOpen"
                             @keydown.escape.window="if (printFailOpen) printFailOpen = false"
                             @click.self="printFailOpen = false"
                            ...
                            <div x-show="printFailOpen"
                                 ...
                                 role="dialog" aria-modal="true" aria-label="Error al registrar el pago"
                                 class="relative bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-sm p-8 text-center">
                                <button type="button" aria-label="Cerrar ventana de error"
                                        class="absolute top-3 right-3 inline-flex items-center justify-center w-9 h-9 rounded-full text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-red-500"
                                        @click="printFailOpen = false">
                                ...
                                <div class="text-base text-gray-600 dark:text-gray-300 mb-6" x-text="printFailMsg || 'No se pudo registrar el pago. Intenta de nuevo.'"></div>
                                <button type="button" class="w-full py-2.5 rounded-lg bg-red-500 hover:bg-red-600 text-white font-medium transition-colors" @click="printFailOpen = false">Cerrar</button>
                            </div>
                        </div>
                        </template>
                        ...
                        <div class="receipt">
                            ...
                            <div class="receipt-grid">
                                ...
                                <div class="hidden">Costo de reconexión</div><div class="hidden" x-text="moneda(recargoMonto())"></div>
                                <div class="hidden">Pago por adelantado</div><div class="hidden" x-text="form.prepay==='si' ? 'SÍ' : 'NO'"></div>
                ...
                <div class="lg:col-span-1 not-print mt-4 lg:mt-0 hidden">
                    ...
                    <div class="bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 shadow-xl sm:rounded-2xl border border-gray-200 dark:border-gray-700 flex flex-col overflow-hidden" aria-label="Lista de deudas históricas">
                        <!-- Header -->
                        <div class="py-3 px-4 text-white" style="background:linear-gradient(135deg,#16a34a,#15803d);border-radius:0.75rem 0.75rem 0 0">
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-2">
                                    <div class="w-7 h-7 rounded-lg flex items-center justify-center" style="background:rgba(255,255,255,.15)">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/>
                                        </svg>
                                    </div>
                                    <div>
                                        <h3 class="text-sm font-bold leading-tight">Morosos</h3>
                                        <p class="text-xs" style="color:rgba(255,255,255,.7)">Excluye mes actual</p>
                                    </div>
                                </div>
                                <div class="text-right" style="background:rgba(255,255,255,.15);border-radius:0.5rem;padding:0.375rem 0.625rem">
                                    <div class="text-xs" style="color:rgba(255,255,255,.7)">Total</div>
                                    <div class="font-bold text-base leading-tight" x-text="morosos.length"></div>
                                </div>
                            </div>
                        </div>
                        
                        <div class="flex-1 overflow-y-auto p-3 space-y-3 custom-scrollbar" style="max-height: calc(100vh - 420px);">
                            <template x-if="loadingMorosos">
                                <div class="py-12 text-center space-y-4" role="status" aria-live="polite">
                                    <div class="flex justify-center">
                                        <svg class="animate-spin h-10 w-10 text-red-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" aria-hidden="true">
                                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                        </svg>
                                    </div>
                                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Cargando lista de morosos...</p>
                                </div>
                            </template>

                            <template x-for="m in paginatedMorosos" :key="m.numero">
                                <div class="relative flex items-center gap-2 bg-white dark:bg-gray-800 rounded-lg border border-gray-100 dark:border-gray-700 hover:border-red-200 hover:shadow-md transition-all duration-200 overflow-hidden mb-1.5">
                                    <div class="absolute left-0 top-0 bottom-0 w-1 flex-shrink-0" style="background:linear-gradient(to bottom,#ef4444,#e11d48)"></div>
                                    <button @click="form.numero = m.numero; buscar()"
                                        class="flex-1 text-left pl-4 pr-2 py-2.5 min-w-0">
                                        <div class="flex items-center gap-2 mb-0.5">
                                            <span class="text-xs font-bold px-1.5 py-0.5 rounded-md flex-shrink-0"
                                                  style="background:#eef2ff;color:#4338ca" x-text="'#' + m.numero"></span>
                                            <span class="text-sm font-semibold text-gray-800 dark:text-gray-100 truncate" x-text="m.nombre"></span>
                                        </div>
                                        <div class="flex items-center gap-1.5 mt-0.5">
                                            <svg class="w-3 h-3 flex-shrink-0" style="color:#f59e0b" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                                            </svg>
                                            <span class="text-xs text-gray-400 truncate" x-text="m.desde_periodo"></span>
                                        </div>
                                    </button>
                                    <span class="flex-shrink-0 text-xs font-bold text-white px-2 py-1 rounded-lg mr-1"
                                          style="background:linear-gradient(135deg,#ef4444,#dc2626)"
                                          x-text="moneda(m.pendiente)"></span>
                                    <button @click="confirmarTransferencia(m)"
                                        class="flex-shrink-0 mr-2 p-1.5 rounded-lg text-white transition-opacity"
                                        style="background:linear-gradient(135deg,#10b981,#059669)"
                                        title="Marcar transferencia recibida">
    ...
    function pagosRecibo(){
        ...
        return {
            ...
            cobradoresBg: [
                'background:oklch(0.93 0.03 343);color:oklch(0.45 0.15 343)',
            ...
            async fetchMorosos(){
                ...
                try {
                    ...
                    const response = await fetch('{{ route('pagos.dashboard.morosos') }}?month=' + month, {
                        headers: {'Accept': 'application/json'}
                    });
                ...
                try{
                    ...
                    if(r.ok && j?.ok){
                        ...
                        if(isFinite(m) && m>0){
                            ...
                            this.adeudo = { desde_periodo:j.desde_periodo, desde_label:j.desde_mes_label||'', meses:meses, pendiente:0, recargo:serverRecargo, pagado_parcial:0, lista_meses: j.lista_meses || [] };
                        }
                        if (!this.pagadoMesActual && !this.alCorriente && !this.recargoManual) {
            ...
            async printThermal(){
                ...
                // El ticket impreso replica exactamente los campos del de Admin.
                // Estas 3 líneas quedan declaradas pero NO se insertan en el
                // ticket (Admin no las imprime). Se conservan para poder
                // reactivarlas sin volver a escribirlas.
