<x-app-sidebar>
    <x-slot name="headerTitle">Pagos - Rosalito</x-slot>

    <div x-data="rosalitoPagos()" x-init="init && init()">
        <div class="max-w-6xl mx-auto sm:px-4 lg:px-8 py-6">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 not-print">
                        <div class="lg:col-span-2 bg-gray-50 dark:bg-gray-700 rounded-xl p-4 sm:p-5 shadow-inner">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div class="sm:col-span-2">
                                    <label for="numero" class="block text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-1">Número de servicio</label>
                                    <div class="flex gap-2">
                                        <input id="numero" type="number" placeholder="Ingresa el número de servicio..."
                                            class="form-input flex-1 rounded-lg border-gray-300 dark:border-gray-600 shadow-sm text-base focus:border-indigo-400 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 min-h-[44px]"
                                            x-model.trim="form.numero" :disabled="readOnlyMode"
                                            @change="!readOnlyMode && buscar()" @keydown.enter.prevent="!readOnlyMode && buscar()">
                                        <button @click="buscar()" class="btn btn-primary px-4 min-h-[44px]" :disabled="readOnlyMode">
                        ...
                        <div class="flex flex-col items-stretch gap-3 bg-gray-50 dark:bg-gray-700 rounded-xl p-4 sm:p-5 shadow-inner">
                            <h3 class="text-sm font-bold uppercase tracking-wider text-gray-400 dark:text-gray-300 text-center mb-2">Acciones</h3>

                            <a href="{{ route('rosalito.historial') }}" class="w-full px-4 py-3 rounded-lg bg-gray-500 text-white font-medium shadow hover:shadow-md hover:brightness-110 active:scale-95 transition-all duration-150 min-h-[48px] flex items-center justify-center gap-2">
    ...
    function rosalitoPagos(){
        ...
        const defaultLayout = (()=>{
            const baseDefaults = {
                logo:{ left:60, top:130, w:680 },
                mes:{ left:580, top:118, w:180 },
                sello:{ left:600, top:40, w:120 },
                alerta:{ left:260, top:146, w:280 },
                cuenta:{ left:120, top:430, w:460 },
                reportes:{ left:140, top:470, w:420 },
        ...
        return {
            ...
            async loadServerLayout(){
                try{
                    const r = await fetch('{{ route('rosalito.recibos.layout.get') }}', {
            ...
            async init(){
                ...
                try{
                    const r = await fetch('{{ route('rosalito.prepay.settings') }}', { headers:{'Accept':'application/json'} });
                    const j = await r.json();
            ...
            async fetchAdeudo(){
                ...
                if(!numero) return;
                try{
                    const r = await fetch('{{ route('rosalito.recibos.deuda') }}?numero='+encodeURIComponent(numero), { headers:{'Accept':'application/json'} });
                    const j = await r.json();
                    if(r.ok && j?.ok){
            ...
            async fetchPagoAnterior(){
                ...
                try{
                    let url = '{{ route('rosalito.recibos.prev') }}?numero='+encodeURIComponent(this.form.numero);
                    if (this.ref && this.ref.id) {
            ...
            async verificarCorteActivo(){
                try {
                    ...
                    const response = await fetch('{{ route('rosalito.corte.activo') }}', {
                        method: 'GET',
                        headers: {
                            'X-CSRF-TOKEN': token,
            ...
            async emitirFactura(){
                try{
                    const token = document.querySelector('meta[name=csrf-token]')?.getAttribute('content') || '';
                    const r = await fetch('{{ route('rosalito.recibos.facturas.store') }}', {
                        method:'POST',
                        headers:{
                    ...
                    if(r.ok && j?.ok){
                        ...
                        try{
                            const vr = await fetch('{{ url('rosalito/recibos/facturas') }}/'+j.id+'/verificar');
...
<\/script>
            ...
            async buscar(){
                ...
                try{
                    const r = await fetch('{{ route('rosalito.recibos.lookup') }}?numero='+encodeURIComponent(this.form.numero));
                    const j = await r.json();
                    if(!j.ok){
            ...
            async fetchPrepayStatus(){
                ...
                try{
                    const r = await fetch('{{ route('rosalito.recibos.prepay.status') }}?numero='+encodeURIComponent(numero), { headers:{'Accept':'application/json'} });
                    const j = await r.json();
                    if(r.ok && j?.ok){
                        const d = j.data || {};
