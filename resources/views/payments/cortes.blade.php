            ...
            <div class="bg-white shadow-sm rounded-lg p-6">
                ...
                <div class="mt-4 overflow-x-auto">
                    <table class="min-w-full text-sm text-left text-gray-600">
                        <thead class="bg-amber-50 text-xs uppercase text-amber-800"><tr><th class="px-4 py-3">Corte</th><th class="px-4 py-3">Cajero</th><th class="px-4 py-3">Inicio</th><th class="px-4 py-3">Cobros</th><th class="px-4 py-3">Recaudado</th><th class="px-4 py-3">Comisión</th><th class="px-4 py-3">Total a entregar</th></tr></thead>
                        <tbody>
                            @forelse($cortesActivos as $corte)
                                <tr class="border-b"><td class="px-4 py-3 font-medium">#{{ $corte->id }}</td><td class="px-4 py-3">{{ $corte->user?->name ?? 'Sin usuario' }}</td><td class="px-4 py-3">{{ $corte->fecha_inicio?->format('d/m/Y H:i') }}</td><td class="px-4 py-3">{{ $corte->total_pagos_actual }}</td><td class="px-4 py-3 font-semibold text-green-600">${{ number_format((float) $corte->total_recaudado_actual, 2) }}</td><td class="px-4 py-3 text-blue-600">${{ number_format((float) $corte->comision_recibos_actual, 2) }}</td><td class="px-4 py-3 font-semibold text-green-700">${{ number_format((float) $corte->total_entregar_actual, 2) }}</td></tr>
                            @empty
                                <tr><td colspan="7" class="px-4 py-6 text-center text-gray-500">No hay cortes activos.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
