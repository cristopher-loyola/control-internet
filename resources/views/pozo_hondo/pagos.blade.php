<x-app-sidebar>
    ...
    <div x-data="pozoHondoPagos()" x-init="init && init()">
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 not-print">
                        ...
                        <div class="flex flex-col items-stretch gap-3 bg-gray-50 dark:bg-gray-700 rounded-xl p-4 sm:p-5 shadow-inner">
                            ...
                            <a href="{{ route('pozo_hondo.historial') }}" class="w-full px-4 py-3 rounded-lg bg-gray-500 text-white font-medium shadow hover:shadow-md hover:brightness-110 active:scale-95 transition-all duration-150 min-h-[48px] flex items-center justify-center gap-2">
    ...
    function pozoHondoPagos(){
        ...
        return {
            ...
            async loadServerLayout(){
                try{
                    const r = await fetch('{{ route('pozo_hondo.recibos.layout.get') }}', {
            ...
            async init(){
                ...
                try{
                    const r = await fetch('{{ route('pozo_hondo.prepay.settings') }}', { headers:{'Accept':'application/json'} });
                    const j = await r.json();
            ...
            async fetchAdeudo(){
                ...
                if(!numero) return;
                try{
                    const r = await fetch('{{ route('pozo_hondo.recibos.deuda') }}?numero='+encodeURIComponent(numero), { headers:{'Accept':'application/json'} });
                    const j = await r.json();
                    if(r.ok && j?.ok){
            ...
            async verificarCorteActivo(){
                try {
                    ...
                    const response = await fetch('{{ route('pozo_hondo.corte.activo') }}', {
                        method: 'GET',
                        headers: {
                            'X-CSRF-TOKEN': token,
            ...
            async emitirFactura(){
                try{
                    const token = document.querySelector('meta[name=csrf-token]')?.getAttribute('content') || '';
                    const r = await fetch('{{ route('pozo_hondo.recibos.facturas.store') }}', {
                        method:'POST',
                        headers:{
                    ...
                    if(r.ok && j?.ok){
                        ...
                        try{
                            const vr = await fetch('{{ url('pozo-hondo/recibos/facturas') }}/'+j.id+'/verificar');
...
<\/script>
            ...
            async buscar(){
                ...
                try{
                    const r = await fetch('{{ route('pozo_hondo.recibos.lookup') }}?numero='+encodeURIComponent(this.form.numero));
                    const j = await r.json();
                    if(!j.ok){
            ...
            async fetchPrepayStatus(){
                ...
                try{
                    const r = await fetch('{{ route('pozo_hondo.recibos.prepay.status') }}?numero='+encodeURIComponent(numero), { headers:{'Accept':'application/json'} });
                    const j = await r.json();
                    if(r.ok && j?.ok){
                        const d = j.data || {};
