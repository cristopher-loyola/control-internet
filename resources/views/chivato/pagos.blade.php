<x-app-sidebar>
    <x-slot name="headerTitle">Pagos - Chivato</x-slot>

    <div x-data="chivatoPagos()" x-init="init && init()">
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 not-print">
                        ...
                        <div class="flex flex-col items-stretch gap-3 bg-gray-50 dark:bg-gray-700 rounded-xl p-4 sm:p-5 shadow-inner">
                            ...
                            <a href="{{ route('chivato.historial') }}" class="w-full px-4 py-3 rounded-lg bg-gray-500 text-white font-medium shadow hover:shadow-md hover:brightness-110 active:scale-95 transition-all duration-150 min-h-[48px] flex items-center justify-center gap-2">
    ...
    function chivatoPagos(){
        ...
        return {
            ...
            async loadServerLayout(){
                try{
                    const r = await fetch('{{ route('chivato.recibos.layout.get') }}', {
            ...
            async fetchAdeudo(){
                ...
                if(!numero) return;
                try{
                    const r = await fetch('{{ route('chivato.recibos.deuda') }}?numero='+encodeURIComponent(numero), { headers:{'Accept':'application/json'} });
                    const j = await r.json();
                    if(r.ok && j?.ok){
            ...
            async fetchPagoAnterior(){
                ...
                try{
                    let url = '{{ route('chivato.recibos.prev') }}?numero='+encodeURIComponent(this.form.numero);
                    if (this.ref && this.ref.id) {
            ...
            async emitirFactura(){
                try{
                    const token = document.querySelector('meta[name=csrf-token]')?.getAttribute('content') || '';
                    const r = await fetch('{{ route('chivato.recibos.facturas.store') }}', {
                        method:'POST',
                        headers:{
...
<\/script>
            ...
            async fetchPrepayStatus(){
                ...
                try{
                    const r = await fetch('{{ route('chivato.recibos.prepay.status') }}?numero='+encodeURIComponent(numero), { headers:{'Accept':'application/json'} });
                    const j = await r.json();
                    if(r.ok && j?.ok){
                        const d = j.data || {};
