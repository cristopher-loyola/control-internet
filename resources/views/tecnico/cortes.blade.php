<x-app-tecnico-layout title="CORTES">
    <x-slot name="header">
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                    <div class="flex items-center gap-4">
                        <form action="{{ route('tecnico.cortes.index') }}" method="GET" class="flex gap-2">
                    ...
                    <div class="flex gap-2">
                        <a href="{{ route('tecnico.cortes.export.pdf') }}?{{ http_build_query(request()->only(['q', 'zona', 'estado'])) }}" class="btn btn-danger flex items-center gap-2" title="Exportar a PDF los usuarios por cortar">
                            📄 PDF
                        </a>
                        <a href="{{ route('tecnico.cortes.export.csv') }}?{{ http_build_query(request()->only(['q', 'zona', 'estado'])) }}" class="btn btn-success flex items-center gap-2" title="Exportar a Excel (CSV) los usuarios por cortar">
                            📊 Excel
                        </a>
            ...
            return {
                ...
                async updateUser(id, value, field) {
                    try {
                        ...
                        const r = await fetch(`{{ url('/tecnico/cortes') }}/${id}/update`, {
                            method: 'POST',
                            headers: {
                ...
                async addCortador() {
                    ...
                    try {
                        const token = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
                        const r = await fetch("{{ route('tecnico.cortes.cortadores.store') }}", {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'Accept': 'application/json',
                ...
                async removeCortador(id) {
                    ...
                    try {
                        const token = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
                        const url = "{{ route('tecnico.cortes.cortadores.destroy', ['id' => 'PLACEHOLDER']) }}".replace('PLACEHOLDER', id);
